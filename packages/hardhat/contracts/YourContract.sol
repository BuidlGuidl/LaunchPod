...
contract YourContract is AccessControl, ReentrancyGuard {
	...
	function modifyAdminRole(
		...
		if (shouldGrant) {
			if (flowingCreators[adminAddress].cap != 0)
    ...
    emit PrimaryAdminTransferred(primaryAdmin, newPrimaryAdmin);
	}
	...
	function addCreatorFlow(
		...
		flowingCreators[_creator] = CreatorFlowInfo(
			_cap,
			block.timestamp - CYCLE
		);
	...
	function validateCreatorInput(
		...
		if (_cap < MINIMUM_CAP && !isERC20)
			revert BelowMinimumCap(_cap, MINIMUM_CAP);
		if (_cap < MINIMUM_ERC20_CAP && isERC20)
			revert BelowMinimumCap(_cap, MINIMUM_ERC20_CAP);
	...
	function updateCreatorFlowCapCycle(
		...
		if (_newCap < MINIMUM_CAP && !isERC20)
			revert BelowMinimumCap(_newCap, MINIMUM_CAP);
		if (_newCap < MINIMUM_ERC20_CAP && isERC20)
			revert BelowMinimumCap(_newCap, MINIMUM_ERC20_CAP);
	...
	function flowWithdraw(
		...
		if (!isERC20) {
			...
			uint256 contractFunds = IERC20(tokenAddress).balanceOf(
				address(this)
			);
			if (contractFunds < _amount) {
				revert InsufficientFundsInContract(_amount, contractFunds);
